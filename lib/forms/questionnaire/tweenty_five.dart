...
class _TweentyFiveState extends State<TweentyFive> {
  ...
  void _submitForm(Interview states) async {
    if (_formKey.currentState.validate()) {
      ...
      double tot = double.parse(_3Controller.text) +
          double.parse(_4Controller.text) +
          double.parse(_5Controller.text) +
          double.parse(_6Controller.text) +
          double.parse(_7Controller.text);
      double tot2 = double.parse(_2Controller.text);
      ...
      if (tot == tot2) {
        ...
        await Provider.of<InterviewModel>(context, listen: false)
            .addSection(states);
        dataExist ? showTopShortToast("Edited") : null;
        setState(() {
          dataExist = true;
        });
      } else {
        showTopShortToast("The inputs are not adding up ${tot} != ${tot2}");
      }
    }
  }

  _totalInput() {
    double foo = double.parse(_3Controller.text) +
        double.parse(_4Controller.text) +
        double.parse(_5Controller.text) +
        double.parse(_6Controller.text) +
        double.parse(_7Controller.text);
  ...
  void showTopShortToast(String message) {
